 ...
 * the functions can be called only in GENERIC_WRITE mode after BLCreateFile, before the first BLWriteObject
 * BLF: the document is written as BL_OBJ_TYPE_APP_TEXT (APPTEXT object of the source BL_APPTEXT_METADATA)
 * other formats (via external provider): the document is stored as a string using BLSetProviderOptionStr()
 * the function can be called multiple times to store multiple metadata documents
 * \param[in] hFile     file void *r returned by BLCreateFile(GENERIC_WRITE)
 * \param[in] pData     buffer containing metadata document to be stored
 * \param[in] dwLength  buffer length
 * \param[in] dwType    metadata type
 * \return TRUE on success, otherwise FALSE
 */
BLAPI(int32_t)	BLWriteMetadata(void * hFile, const char * pData, uint32_t dwLength, uint8_t bType);
/**
 * Metadata document enumeration callback procedure
 * used as an argument of BLEnumMetadata()
 * the callback procedure is called for each metadata document, found in the logging file
 * \param[in] pData	    buffer containing metadata document
 * \param[in] dwLength	buffer length
 * \param[in] bType     metadata type
 * \param[in] pParam    callback custom parameter, passed to BLEnumMetadata()
 * \return TRUE to continue enumerating metadata documents, FALSE to cancel enumeration
 */
typedef int32_t(*BLEnumMetadataCallback)(const char * pData, uint32_t dwLength, uint8_t bType, void * pParam);
/**
 * Enumerates and reads metadata documents, written by BLWriteMetadata()
 * can be called only in GENERIC_READ mode
 * the callback is called for each metadata document, stored in the BLF file
 * \param[in] hFile     file void *r returned by BLCreateFile(GENERIC_READ)
 * \param[in] callback  callback procedure called for each metadata document
 * \param[in] pParam    callback custom parameter, optional, can be NULL
 * \return TRUE on success, otherwise FALSE
 */
BLAPI(int32_t) BLEnumMetadata(void * hFile, BLEnumMetadataCallback callback, void * pParam);
