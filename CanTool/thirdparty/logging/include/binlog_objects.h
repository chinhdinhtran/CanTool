...
#define BL_OBJ_TYPE_VIDEOSYNC                     108   /* VideoSync object */
...
#define BL_OBJ_TYPE_reserved_4                    115

#define BL_OBJ_TYPE_reserved_5                    116

#define BL_OBJ_TYPE_reserved_6                    117
...
#define BL_OBJ_TYPE_reserved_7                    135
...
#define BL_OBJ_TYPE_TUNNEL_PROTOCOL_DECODER_EVENT 138   /* Tunnel protocol decoder event object */
#define BL_OBJ_TYPE_CAN_XL_CHANNEL_FRAME          139   /*CAN, CANFD, CANXL message object on XL channel*/
#define BL_OBJ_TYPE_CAN_XL_CHANNEL_ERRORFRAME   140   /*CAN, CANFD, CANXL error message object on XL channel*/
#define BL_OBJ_TYPE_10BASET1S_CYCLESTATISTIC      141   /* 10BaseT1S cycle statistic object */
...
/*----------------------------------------------------------------------------
|
| CAN XL channel objects
|
-----------------------------------------------------------------------------*/
/* HINT: This structure might be extended in future versions! */

typedef struct VBLCANXLBitTimingData_t
{
  uint64_t mBitTimingConfiguration;
  uint64_t mHardwareChannelSettings;  
} VBLCANXLBitTimingData;

typedef struct VBLCANXLChannelFrame_t
{
  VBLVarObjectHeader mHeader; /* object header */
  struct VBLCANXLBitTimingDataStatic_t
  {
    uint8_t mChannel; /* application channel */
    ...
    uint16_t mBitCount;    /* complete message length in bits */
    uint32_t mID;          /* CAN ID */
    uint8_t  mSDT;          /*SDT field value 8 bit*/    
    uint16_t mDLC;         /*Data length code*/
    uint16_t mDataLength;  /*The data length correspoding to the dlc*/
    uint16_t mSBC;  /*SBC field for CANXL includes StuffCountParity and StuffCount, for CAN and CAN-FD always zero */
    uint16_t mPCRC; /*PCRC field of CAN XL message, for CAN and CAN-FD always zero*/
    uint8_t mVCID;  /*VCID field of CAN XL message, for CAN and CAN-FD always zero*/
    uint32_t mAF;    /*AF field of CAN XL message, for CAN and CAN-FD always zero*/
    uint8_t mStuffCount;       /*StuffCount field for CAN-FD includes StuffCountParity and StuffCount,for CAN and CAN-XL
                                  always       zero */
    uint32_t mFCRC;            /*Frame CRC for CAN, CAN-FD and CAN-XL messages*/
    uint32_t mTimeOffsetBrsNs; /* time offset of brs field */
    uint32_t mTimeOffsetCrcDelNs; /* time offset of crc delimiter field */    
    uint32_t mFlags;
    uint32_t mReserved;
    VBLCANXLBitTimingData mArbitrationData;
    VBLCANXLBitTimingData mFDPhaseData;
    VBLCANXLBitTimingData mXLPhaseData;
  } mStatic;
  struct VBLCANXLBitTimingDataDynamic_t
  {
    BL_LPBYTE mData; /* CAN, CANFD or CAN XL  data dynamic written to the object*/
  } mDynamic;
} VBLCANXLChannelFrame;

typedef struct VBLCANXLChannelErrorFrame_t
{
  VBLVarObjectHeader mHeader; /* object header */
  struct VBLCANXLChannelErrorFrameStatic_t
  {
    uint8_t mChannel;        /* application channel */
    uint32_t mValidityFlags; /* Which fields are valid*/
    uint16_t mErrorCode;     /*Contains Dir, ErrorReason*/
    uint16_t mErrorPosition; /*Error position in frame (without stuff bits)*/
    uint32_t mMessageFlags;  /*CAN-FD, CAN-XL specific flags*/
    uint32_t mFrameLength;   /* message length in ns until the error position*/

    uint32_t mID;    
    uint8_t  mADS;
    uint8_t  mSDT;            /*SDT field value 8 bit*/
    uint16_t mDLC;        /*Data length code*/
    uint16_t mDataLength; /* Valid payload length of mData */
    uint16_t mSBC;  /*SBC field for CANXL includes StuffCountParity and StuffCount, for CAN and CAN-FD always zero */
    uint16_t mPCRC; /*PCRC field of CAN XL message, for CAN and CAN-FD always zero*/
    uint8_t  mVCID;  /*VCID field of CAN XL message, for CAN and CAN-FD always zero*/
    uint32_t mAF;   /*AF field of CAN XL message, for CAN and CAN-FD always zero*/
    ...
    uint8_t  mFCP;
    uint8_t  mDAS;
    uint32_t mTimeOffsetArb2DataNs; /* time offset of brs field */
    uint32_t mTimeOffsetData2ArbNs; /* time offset of crc delimiter field */
    uint32_t mReserved;
    VBLCANXLBitTimingData mArbitrationData;
    VBLCANXLBitTimingData mFDPhaseData;
    VBLCANXLBitTimingData mXLPhaseData;
  } mStatic;
  struct VBLCANXLChannelErrorFrameDynamic_t
  {
    BL_LPBYTE mData; /* CAN, CANFD or CAN XL  data dynamic written to the object*/
  } mDynamic;
} VBLCANXLChannelErrorFrame;
...
/*----------------------------------------------------------------------------
|
| Ethernet 10BaseT1S CycleStatistics
|
-----------------------------------------------------------------------------*/

typedef struct VBL10BaseT1SCycleStatistic_t
{
  VBLObjectHeader mHeader; /* object header */
  uint16_t mChannel;
  uint64_t mCycleCounter;
  uint16_t mObservedEmptyCycles;
  uint16_t mObservedCycles;
  uint32_t mCurrShortestCycleTime;
  uint32_t mCurrLongestCycleTime;
  uint32_t mFlags;
  uint16_t mHardwareChannel;
} VBL10BaseT1SCycleStatistic;
...
#define BL_APPID_VITESCO          228
#define BL_APPID_PMX_ENGINEERING_GMBH 229
#define BL_APPID_HONDA            230
#define BL_APPID_PREH             231
...
typedef struct VBLFileStatisticsEx_t
{
  ...
  uint32_t      mLastObjectTimeNs;                 /* ns part of the last object time (everything below ms resolution) */
  uint32_t      mReserved[15];                     /* reserved */
} VBLFileStatisticsEx;
...
|
-----------------------------------------------------------------------------*/
typedef struct VBLVideoSync_t
{
  VBLObjectHeader mHeader;                     /* object header */
  uint64_t        mFrameNumber;                /* frame number*/
} VBLVideoSync;
...
typedef struct VBLTestStructure_t
{
  ...
  BL_LPWSTR       mExecutingObjectName;        /* name of the executing test module or test configuration as shown in CANoe (char16_t) */
  BL_LPWSTR       mName;                       /* name of structure element (can change between begin/end when using CAPL function TestCaseTitle or similar (char16_t) */
...
/*----------------------------------------------------------------------------
|
| Tunnel Protocol Decoder Event
|
-----------------------------------------------------------------------------*/

typedef struct VBLTunnelProtocolDecoderEvent_t
{
  VBLObjectHeader mHeader; /* object header */
  uint32_t mDecoderType;
  uint32_t mEventType;
  uint32_t mDeviceId;
  uint32_t mStreamId;
  uint32_t mInterfaceId;
} VBLTunnelProtocolDecoderEvent;
